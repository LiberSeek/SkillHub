...
- Use `wan` for Wan 3.0 all-in-one video generation. The supported models are `wan3.0-video` and `wan3.0-video-prime`; override with `--model` when selecting Prime. Wan accepts text in `prompt` plus official `first_frame`, `last_frame`, `reference_image`, `reference_video`, and `reference_audio` media inputs. Use the exact provider media names in custom JSON requests. Output duration is 1-30 seconds.
